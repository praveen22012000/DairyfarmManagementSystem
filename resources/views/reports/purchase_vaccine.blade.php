...
<div class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md mt-6">
    <h1 style="text-align:center">Vaccine Items Report</h1>
...
@if (!empty($purchaseVaccineData) && count($purchaseVaccineData))
    <form method="GET" action="{{ route('vaccine_items.report.pdf') }}" target="_blank">
...
@if(count($purchaseVaccineData))
<p>From <strong>{{ $start }}</strong> to <strong>{{ $end }}</strong></p>
      <table class="table table-bordered">
        <thead class="thead-dark">
            <tr>
                <th class="px-4 py-2 border">Vaccine Name</th>
                <th class="px-4 py-2 border">Purchase Quantity</th>
            </tr>
        </thead>
        <tbody>
            @foreach($purchaseVaccineData as $item)
                <tr>
                    <td class="px-4 py-2 border">{{ $item->vaccine_name }}</td>
                    <td class="px-4 py-2 border">{{ number_format($item->total_purchase_quantity, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
