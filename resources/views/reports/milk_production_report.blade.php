...
<div class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md mt-6">
    <h1 style="text-align:center">Milk Production Report</h1>
...
@if (!empty($milkData) && count($milkData))
    <form method="GET" action="{{ route('milk.production.report.pdf') }}" target="_blank">
                ...
                <tbody>
                    @foreach($milkData as $data)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 border-b">{{ number_format($data->total_milk, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
