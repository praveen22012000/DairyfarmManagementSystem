...
<div class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md mt-6">
    <h1 style="text-align:center">Delivered Retailor Order Products</h1>
...
@if (!empty($totalDeliveredOrders) && count($totalDeliveredOrders))
    <form method="GET" action="{{ route('delivered.retailor_order_items.report.pdf') }}" target="_blank">
...
@if(count($totalDeliveredOrders))
<p>From <strong>{{ $start }}</strong> to <strong>{{ $end }}</strong></p>
      <table class="table table-bordered">
        <thead class="thead-dark">
            <tr>
                <th class="px-4 py-2 border">Product Name</th>
                <th class="px-4 py-2 border">Delivered Quantity</th>
            </tr>
        </thead>
        <tbody>
            @foreach($totalDeliveredOrders as $item)
                <tr>
                    <td class="px-4 py-2 border">{{ $item->product_name }}</td>
                    <td class="px-4 py-2 border">{{ number_format($item->total_delivered_quantity, 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
