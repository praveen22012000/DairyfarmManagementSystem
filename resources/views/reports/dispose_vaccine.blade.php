...
<div class="max-w-5xl mx-auto p-6 bg-white rounded-xl shadow-md mt-6">
    <h1 style="text-align:center">Dispose Vaccine Items Report</h1>
...
@if (!empty($disposeVaccineData) && count($disposeVaccineData))
    <form method="GET" action="{{ route('dispose_vaccine_items_pdf.report') }}" target="_blank">
...
<p>From <strong>{{ $start }}</strong> to <strong>{{ $end }}</strong></p>
      <table class="table table-bordered">
        ...
        <tbody>
            @foreach($disposeVaccineData as $item)
                <tr>
