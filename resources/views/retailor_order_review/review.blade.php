...
@endif
        ...
        <br>
            <div>
                ...
                <input type="text" name="delivery_address" id="delivery_address" readonly placeholder="Enter your delivery address" class="form-control rounded" value="{{ $retailororder->delivery_address}}" >
            ...
            <table id="milkTable" class="table">
                <thead class="thead-dark">
                    <tr>
                        ...
                        <th class="border-b px-6 py-4 text-left">Availability</th>
                    </tr>
                </thead>
                <tbody>
                    @php
                    ...
                    <select name="product_id[]" disabled class="border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                        <option value="">Select the Product</option>
                            @foreach($milk_products as $milk_product)
                                <option value="{{$milk_product->id}}" 
                                    {{ ( $item->product_id == $milk_product->id) ? 'selected' : '' }}
                    ...
                    @error('product_id') <span class="text-danger">{{ $message }}</span> @enderror
                        ...
                        <td class="border-t px-6 py-4">
                            <input type="number" readonly name="ordered_quantity[]" class="form-control rounded" value="{{ $item->ordered_quantity }}">
                            @error('ordered_quantity') <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                    ...
                    @if ($item->is_available)
                        <span style="color: green;">Available Stock</span>
                    @else
                        <span style="color: red;">Not Enough Stock</span>
                    @endif
                        </td>
