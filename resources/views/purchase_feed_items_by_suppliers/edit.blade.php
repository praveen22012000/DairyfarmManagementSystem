...
<div class="col-md-12">
    ...
    <form action="{{route('purchase_feed_items.update',$purchasefeeditem->id)}}" method="POST" class="space-y-8">
        @csrf
...
@endif
        ...
        <div class="mb-4">
                <label for="supplier_id" class="block text-gray-700 font-medium" >Suppliers</label>
                    ...
                    <option value="">Select the Supplier</option>
                        @foreach($suppliers as $supplier)
                            ...
                            {{$purchasefeeditem->purchase_feed->supplier_id== $supplier->id ?'selected':''}}
            ...
            <div>
                ...
                <br>
                <input type="date" name="purchase_date" id="purchase_date" class="form-control rounded" value="{{$purchasefeeditem->purchase_feed->purchase_date}}">
                ...
                <tr class="purchase-row">
                        ...
                        <td class="border-t px-6 py-4 text-left text-gray-800">
                                <select name="feed_id" id="feed_id" class="border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                 ...
                                 {{$purchasefeeditem->feed_id == $feed->id ? 'selected':''}}
                        ...
                        <td>
                            <input type="number" class="form-control" name="purchase_quantity" value="{{$purchasefeeditem->purchase_quantity}}" style="width: 100px;">
                            @error("purchase_quantity") <span class="text-danger">{{ $message }}</span> @enderror  
                        </td>

                        <td>
                            <input type="number" class="form-control" name="unit_price" value="{{$purchasefeeditem->unit_price}}"  style="width: 100px;">
                        ...
                        <td>
                            <input type="date" name="manufacture_date" value="{{$purchasefeeditem->manufacture_date}}"  class="border border-gray-400 rounded-lg px-2 py-1 w-28 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            @error("manufacture_date") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>

           
                        <td>
                            <input type="date" name="expire_date" value="{{$purchasefeeditem->expire_date}}"  class="border border-gray-400 rounded-lg px-2 py-1 w-28 focus:ring-2 focus:ring-blue-500 focus:outline-none">
