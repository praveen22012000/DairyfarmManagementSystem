...
<div class="col-md-12">
    <h1 class="text-3xl font-bold mb-10 text-center text-gray-700">Purchase Vaccine Items </h1>

    <form action="{{route('purchase_vaccine_items.update',$purchasevaccineitem->id)}}" method="POST" class="space-y-8">
        @csrf
...
@endif
        ...
        <div class="mb-4">
                <label for="supplier_id" class="block text-gray-700 font-medium" >Suppliers</label>
                    ...
                    <option value="">Select the Supplier</option>
                        @foreach($suppliers as $supplier)
                            ...
                            {{$purchasevaccineitem->purchase_vaccine->supplier_id== $supplier->id ?'selected':''}}
            ...
            <div>
                ...
                <br>
                <input type="date" name="purchase_date" id="purchase_date" class="form-control rounded" value="{{$purchasevaccineitem->purchase_vaccine->purchase_date}}">
            ...
            <table  class="table" id="purchaseTable">
                ...
                <tr class="purchase-row">
                        ...
                        <td class="border-t px-6 py-4 text-left text-gray-800">
                                <select name="vaccine_id" id="vaccine_id" class="border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                    <option value="">Select the Feed</option>
                                            @foreach($vaccines as $vaccine)
                                    <option value="{{$vaccine->id}}"
                                 {{$purchasevaccineitem->vaccine_id == $vaccine->id ? 'selected':''}}
                        ...
                        <td>
                            <input type="number" class="form-control" name="purchase_quantity" value="{{$purchasevaccineitem->purchase_quantity}}" style="width: 100px;">
                            @error("purchase_quantity") <span class="text-danger">{{ $message }}</span> @enderror  
                        </td>

                        <td>
                            <input type="number" class="form-control" name="unit_price" value="{{$purchasevaccineitem->unit_price}}"  style="width: 100px;">
                        ...
                        <td>
                            <input type="date" name="manufacture_date" value="{{$purchasevaccineitem->manufacture_date}}"  class="border border-gray-400 rounded-lg px-2 py-1 w-28 focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            @error("manufacture_date") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>

           
                        <td>
                            <input type="date" name="expire_date" value="{{$purchasevaccineitem->expire_date}}"  class="border border-gray-400 rounded-lg px-2 py-1 w-28 focus:ring-2 focus:ring-blue-500 focus:outline-none">
