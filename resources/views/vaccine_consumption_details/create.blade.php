...
<div class="col-md-12">
    ...
    <form action="{{route('vaccine_consume_items.store')}}" method="POST" class="space-y-8">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                ...
                <input type="date" name="vaccination_date" id="vaccination_date" class="form-control rounded" value="{{old('vaccination_date')}}">
        ...
        <div class="overflow-x-auto">
            <table id="vaccineTable" class="table">
                ...
                <tbody>
                    @php
                        $oldAnimalIds = old('animal_id', []);
                        $oldVaccineIds = old('vaccine_id', []);
                        $oldVaccinationItemIds = old('vaccination_item_id', []);
                        $oldConsumeQuantities = old('consumed_quantity', []);
                        $rowCount = max(count($oldAnimalIds), 1);
                    @endphp
                    ...
                    <tr class="milk-row">
                            ...
                            <select name="animal_id[]" class="animal-select border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                ...
                                @foreach($animals as $animal)
                                    <option value="{{ $animal->id }}"
                                        {{ (isset($oldAnimalIds[$i]) && $oldAnimalIds[$i] == $animal->id) ? 'selected' : '' }}>
                                        {{ $animal->animal_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error("animal_id.$i") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                            ...
                            <select name="vaccine_id[]" class="vaccine-select border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                                ...
                                @foreach($vaccinations as $vaccination)
                                    <option value="{{ $vaccination->id }}"
                                        {{ (isset($oldVaccineIds[$i]) && $oldVaccineIds[$i] == $vaccination->id) ? 'selected' : '' }}>
                                        {{ $vaccination->vaccine_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error("vaccine_id.$i") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>
                        ...
                        <td class="border-t px-6 py-4 text-left text-gray-800">
                            <select name="vaccination_item_id[]" class="purchase-item-select border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none" 
                                    {{ empty($oldVaccineIds[$i]) ? 'disabled' : '' }}>
                                ...
                                @foreach($vaccination_items as $vaccination_item)
                                    <option value="{{ $vaccination_item->id }}"
                                        ...
                                        {{ (isset($oldVaccinationItemIds[$i]) && $oldVaccinationItemIds[$i] == $vaccination_item->id) ? 'selected' : '' }}>
                                        {{ $vaccination_item->id.'|'. $vaccination_item->vaccine->vaccine_name.'|'. $vaccination_item->expire_date }}
                                    </option>
                                @endforeach
                            </select>
                            @error("vaccination_item_id.$i") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>

                        <td class="border-t px-6 py-4 text-left text-gray-800">
                            <input type="number" class="stock-quantity form-control" name="quantity[]" readonly style="width: 90px;" 
                                   value="{{ $vaccination_items->firstWhere('id', $oldVaccinationItemIds[$i] ?? '')->stock_quantity ?? '' }}">
                        </td>

                        <td class="border-t px-6 py-4">
                            <input type="text" name="consumed_quantity[]" class="form-control rounded" 
                                   value="{{ $oldConsumeQuantities[$i] ?? '' }}">
        ...
        <div class="flex justify-center">
            <button type="submit" class="btn btn-success mt-3">Save Vaccine Consumption Record</button>
        </div>
    </form>
</div>
@endsection
