...
<div class="col-md-12">
    <h1 class="text-3xl font-bold mb-10 text-center text-gray-700">Dispose Feed Items</h1>

    <form action="{{route('dispose_feed_items.store')}}" method="POST" class="space-y-8">
        @csrf
...
@endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
                ...
                <input type="date" name="dispose_date" id="date" class="form-control rounded" value="{{$disposefeeditem->dispose_feed_details->dispose_date}}" >
            ...
            <div>
                ...
                <input type="time" name="dispose_time" id="time" class="form-control rounded" value="{{$disposefeeditem->dispose_feed_details->dispose_time}}" >
                ...
                <tbody>
                    ...
                    <tr  class="milk-row">
                        <td class="border-t px-6 py-4 text-left text-gray-800">
                            <select name="purchase_feed_item_id" id="purchase_feed_item_id" class="border border-gray-400 rounded-lg px-4 py-2 w-full focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            <option value="">Select the FeedItem</option>
                                @foreach($purchaseFeedItems as $purchaseFeedItem)
                                    <option value="{{$purchaseFeedItem->id}}" 
                                  {{$disposefeeditem->purchase_feed_item_id==$purchaseFeedItem->id ? 'selected' : '' }}
                                       data-stock_quantity="{{ $purchaseFeedItem->stock_quantity }}">
                                        {{ $purchaseFeedItem->id.' | '.$purchaseFeedItem->feed->feed_name.' | '.$purchaseFeedItem->manufacture_date }}
                                    </option>
                                @endforeach 
                        ...
                        <td class="border-t px-6 py-4">
                            <input type="text" name="dispose_quantity" value="{{$disposefeeditem->dispose_quantity}}" class="form-control"  style="width: 80px;">
                            @error("dispose_quantity") <span class="text-danger">{{ $message }}</span> @enderror
                        </td>

                        <td>
                        <input type="text" name="reason_for_dispose" class="form-control" value="{{$disposefeeditem->reason_for_dispose}}"   style="width:300px;" value="">
        ...
        <a href="{{ route('dispose_feed_items.list') }}" class="btn btn-info">Back</a>
...
<script>
   ...
   let today = new Date().toISOString().split("T")[0];
    $("#date").attr("max", today);
     ...
     $("select[name='purchase_feed_item_id']").each(function () {
    ...
    $(document).on("change", "select[name='purchase_feed_item_id']", function () {
        let selectedOption = $(this).find(":selected");
        let stockQuantity = selectedOption.data("stock_quantity");
    ...
    function checkForErrors() {
        ...
        $("#disposeFeedTable tbody tr").each(function () {
    ...
    function updateAddButtonVisibility() {
        ...
        let totalMilkItems = $("#disposeFeedTable tbody tr:first").find("select[name='purchase_feed_item_id[]'] option").length - 1; // Exclude the default option
        let totalRows = $("#disposeFeedTable tbody tr").length;
    ...
    $("#addRow").click(function () {
        ...
        let table = $("#disposeFeedTable tbody");
...
$(document).on("input", "#disposeFeedTable tbody input, #disposeFeedTable tbody select", function () {
    checkForErrors();
});
});
