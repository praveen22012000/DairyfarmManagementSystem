...
<div class="col-md-12">
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('monthly_salary_assign.update',$monthlysalaryassign->id) }}">
        @csrf
        ...
        <div class="form-group">
            ...
            <select name="user_id" id="user_id" class="form-control">
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}"  {{ $monthlysalaryassign->user_id == $user->id ? 'selected' : ''}}
        ...
        <div class="form-group">
            ...
            <input type="text" name="role_id" class="form-control rounded" id="role_id" value="{{ $monthlysalaryassign->user->role->role_name }}" readonly>
    ...
    <input type="month" name="salary_month" 
           value="{{ \Carbon\Carbon::parse($monthlysalaryassign->salary_month)->format('Y-m') }}" 
    ...
    @error('salary_month') <span class="text-danger">{{ $message }}</span> @enderror
        ...
        <div class="form-group">
            ...
            <input type="text" name="custom_salary" class="form-control rounded" value="{{ $monthlysalaryassign->amount_paid }}" id="custom_salary">
...
$(document).ready(function () {// This waits until the entire HTML page is fully loaded before running any JavaScript inside it.
    ...
    // Initialize fields on page load if user is selected
    if ($('#user_id').val()) {
        populateFields();
    }
